...
set "SEA_BIN=%SCRIPT_DIR%openclaw-sea-win-%ARCH%.exe"
if exist "%SEA_BIN%" (
  "%SEA_BIN%" %*
  exit /b %ERRORLEVEL%
)
...
if not exist "%ENTRY%" (
  ...
  echo [hint] run deployment\build-local-runtime.ps1 ^(or deployment/build-local-runtime.sh^) or build SEA binary 1>&2
  exit /b 1
)
